...
func TestGradientRGBEndpoints(t *testing.T) {
	result := gradient.GradientRGB(black, white, 5)
	assert.Equal(t, 5, len(result))
	assert.InDelta(t, black.R, result[0].R, 1e-9)
	assert.InDelta(t, white.R, result[4].R, 1e-9)
}

func TestGradientHSLEndpoints(t *testing.T) {
	result := gradient.GradientHSL(red, blue, 5)
	assert.Equal(t, 5, len(result))
	assert.InDelta(t, red.R, result[0].R, 1e-5)
	assert.InDelta(t, blue.B, result[4].B, 1e-5)
}

func TestGradientLCHEndpoints(t *testing.T) {
	result := gradient.GradientLCH(black, white, 3)
	assert.Equal(t, 3, len(result))
	assert.InDelta(t, black.R, result[0].R, 1e-5)
	assert.InDelta(t, white.R, result[2].R, 1e-5)
}

func TestGradientLABEndpoints(t *testing.T) {
	result := gradient.GradientLAB(red, blue, 3)
	assert.Equal(t, 3, len(result))
	assert.InDelta(t, red.R, result[0].R, 1e-5)
...
func TestGradientEdgeCases(t *testing.T) {
	assert.Nil(t, gradient.GradientRGB(black, white, 0))
	single := gradient.GradientRGB(red, blue, 1)
